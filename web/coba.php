...
<!-- <div class="w3-content" style="max-width:1100px"> -->
  ...
  <div class="w3-container w3-padding-64" id="contact">
    ...
    <form action="save_all.php" target="" method="POST">
      <label>Nama</label>
      <?php
          include 'koneksi.php';
          ...
          <?php
              ...
              $result = mysqli_query($koneksi_db,$sql);
              echo "<select name='id_question'>";
              while ($row = mysqli_fetch_array($result)) {
              echo "<option value='" . $row['id_question'] . "'>" . $row['pertanyaan'] . "</option>";
              }
              echo "</select>";
          ?>
          <br>
          ...
          <?php
              include 'koneksi.php';
              $sql = "SELECT * FROM table_answer";
              $result = mysqli_query($koneksi_db,$sql);
              echo "<select name='id_answer'>";
              while ($row = mysqli_fetch_array($result)) {
              echo "<option value='" . $row['id_answer'] . "'>" . $row['jawaban'] . "</option>";
              }
              echo "</select>";
          ?>
